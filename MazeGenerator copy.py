...
class MazeGenerator:
    ...
    def _get_neighbors(self, x: int,
        ...
        for dw, dh, code, opp_code in directions:
            ...
            if self._is_available(nx, ny):
                ...
                if (self._perfect is False and random.randint(0, 5) == 0
                        and 0 <= ny < self._height and 0 <= nx < self._width):
                    if (self._maze[ny][nx] != 15 and
    ...
    def _walk_rec(self, distance: int, x: int, y: int,
        ...
        for dx, dy, code, way in directions:
            if ((self._maze[y][x] & code) == 0 and 0 <= x+dx < self._width
                    and 0 <= y+dy < self._height and visited[y+dy][x+dx] == 0):
                rec = self._walk_rec(distance-1, x+dx, y+dy, visited, ways + way)
                if rec is not False:
                    return rec
        visited[y][x] = 0
        return False
...
gen = MazeGenerator((5, 5), True, (0, 0), (14,14))
gen.generate()
